...
export const sendVerificationCode = createAsyncThunk(
    ...
    async (
        args: {
            ...
            callback: (
                args:
                    | { type: 'success'; verificationId: string; confirmationResult: any }
                    | {
                          type: 'error';
                          message: string;
                      }
        ...
        if (args.auth && args.auth.type !== LoadingStateTypes.LOADED) return;
        ...
        try {
            ...
            if (args.auth) {
                console.log('hiiiii');
                phoneVerification = await linkWithPhoneNumber(
                    args.auth.user,
                    args.phoneNumber,
                    args.recaptcha
                );
                ...
                console.log('hiiiii2');
                phoneVerification = await signInWithPhoneNumber(
                    firebaseAuth,
                    args.phoneNumber,
                    args.recaptcha
                );
            ...
            if (args.callback)
                args.callback({
                    ...
                    confirmationResult: phoneVerification,
                });
        } catch (error: any) {
            dispatch(
                showToast({
